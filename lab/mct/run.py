...
try:
    ...
    visible_device = [int(v) for v in visible_devices.split(',')][0]
except:
    visible_device = 0
...
class SSL_distance():
    ...
    def __call__(self, x, y, embed_comparison=lambda x, y: KLKL(min_max_norm(x), min_max_norm(y))):
        with torch.no_grad():
            x = x.to(visible_device)
            y = y.to(visible_device)
            return embed_comparison(self.model(x).cpu(), self.model(y).cpu())
...
class EsViT_distance(SSL_distance):
    ...
    EsVit distance
    """
    def __init__(self, chkpt_path='./mct/models/esvit_swin_b.pth', model=vit_base_patch16):
        model = swin_b()
...
class DINOv2_distance(SSL_distance):
    ...
    DINOv2 distance
    """
    def __init__(self, path='./mct',model='dinov2_vitb14'):
        os.environ['TORCH_HOME'] = path+'/models/'
        # os.environ['TORCH_HUB'] = path+'/models/'
...
'EsViT': Compose(
                    [
                        ...
                        transforms.Lambda(lambda x: torch.cat([x, x, x], 0)[:3]),
                        transforms.Resize(size=(224, 224), antialias=True),
                        # transforms.CenterCrop((224, 224)),
                        transforms.Normalize(
                                            mean=[0.002351,0.002351,0.002351],
                                            std=[0.039175,0.039175,0.039175]
                                            ),
                    ]
                    ),
...
import time
from mct.models import FCNN
import cv2
class_id=[ '0','1', '2', '4','5', '6', '8',  '.', 'nan']

def create_model(model_path):
    num_classes=len(class_id)-1   
    dist0 = IMAGE_DISTANCES['EsViT']()
    dist1 = IMAGE_DISTANCES['DINOv2']()
    with open(model_path, 'rb') as fp:
        state = pickle.load(fp)
    state_dict0=state['model1_state']
    state_dict1=state['model0_state']
    fc0 = FCNN(768, num_classes, [1024,1024, 1024], nn.BatchNorm1d, nn.ReLU, dropout=0.2).to(0)
    fc1 = FCNN(768, num_classes, [1024,1024, 1024], nn.BatchNorm1d, nn.ReLU, dropout=0.2).to(0)
    fc0.load_state_dict(state_dict0)
    fc1.load_state_dict(state_dict1)
    fc0.eval()
    fc1.eval()
    return dist0,dist1,fc0,fc1
    
def run_inference(img,dis0,dis1,fc0,fc1):
    t1=time.time()
    image_transform = IMAGE_TRANSFORMS['EsViT']
    ...
    with torch.no_grad():
        x1=dis1.embed(x).to('cuda:0')
             
        x=dis0.embed(x).to('cuda:0')  
        
        x1=fc1(x1)
        x=fc0(x)
        x=torch.nn.Softmax(-1)(x1+x)
        out=x.argmax(1).item()
        out=class_id[out]
    t2=time.time()
    print(t2-t1)
...
# dis0,dis1,fc0,fc1=create_model('./mct/Grid_MCT_IN1K/checkpoints/DINOv2_EsViT_0.1_False_0.pt')
# img_path =os.path.join('./mct/NUMBER', 'images', 'val', 'c310_3_front.png')
# img=cv2.imread(img_path)
# run_inference(img,dis0,dis1,fc0,fc1)
